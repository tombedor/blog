...
output "analytics_url" {
  ...
  ) : "http://${digitalocean_droplet.analytics.ipv4_address}:3000"
}
...
output "listmonk_url" {
  ...
  ) : "http://${digitalocean_droplet.analytics.ipv4_address}:9000"
}

output "ssh_command" {
  description = "SSH command to connect to the droplet"
  value       = "ssh root@${digitalocean_droplet.analytics.ipv4_address}"
}
...
output "next_steps" {
  ...
  value = <<-EOT
    ...
    📊 Umami Analytics: ${var.domain != "" && var.analytics_subdomain != "" ? "https://${var.analytics_subdomain}.${var.domain}" : "http://${digitalocean_droplet.analytics.ipv4_address}:3000"}
       Default credentials: admin / umami
       ⚠️  Change password immediately after first login!

    📧 Listmonk Newsletter: ${var.domain != "" && var.listmonk_subdomain != "" ? "https://${var.listmonk_subdomain}.${var.domain}" : "http://${digitalocean_droplet.analytics.ipv4_address}:9000"}
       Complete setup wizard on first visit

    🔧 SSH Access: ssh root@${digitalocean_droplet.analytics.ipv4_address}

    📝 Check logs:
       ssh root@${digitalocean_droplet.analytics.ipv4_address} 'cd /opt/analytics && docker-compose logs -f'
